...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // XDECoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // XDECoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// XDECoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes XDECoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // XDECoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // XDECoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    /*if(IsProofOfWork())
    {
        ...
        if (vtx[0].vout[1].nValue < XDESupport)
            return error("ConnectBlock() : coinbase does not pay enough to dev addresss");
    }*/

    if (IsProofOfStake())
    {
        // XDECoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // XDECoin: track money supply and mint amount info
...
// XDECoin: total coin age spent in transaction, in the unit of coin-days.
...
// XDECoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // XDECoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // XDECoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // XDECoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // XDECoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // XDECoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // XDECoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // XDECoin: getblocks may not obtain the ancestor block rejected
    ...
    // XDECoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The virtual and very limited XDECoin will take over the world muahaha.";
        CTransaction txNew;
        txNew.nTime = 1410061363;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1410061363;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3357434;
        if(fTestNet)
        {
            block.nNonce   = 3357434;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x93b3a330582edcdd222639940aa1c5117495c065a02d88906e55fab98214fb8a"));
        ...
        // XDECoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // XDECoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // XDECoin: tell downloading node about the latest block if it's
